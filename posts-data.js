  ...
  },
{
    ...
    description: {
      "en": "A complete study note covering bug bounty program types, conduct, scope, report writing, CWE, and CVSS.",
      "zh": "保留原笔记结构，系统梳理漏洞赏金计划类型、行为准则、项目结构、报告写作、CWE 与 CVSS。"
    },
    date: "2026-05-07",
    content: {
      "en": "A complete study note covering bug bounty program types, conduct, scope, report writing, CWE, and CVSS.",
...
<p>HackerOne 对其漏洞赏金平台的描述很贴切：<strong>“持续测试，持续防护（Continuous testing, constant protection）”</strong>。这说明漏洞赏金并不是开发结束后的附加项目，而是可以无缝融入企业现有软件开发生命周期中的安全环节。换句话说，它让安全测试从阶段性行为，变成一种长期运行的机制。</p>
<hr />
...
<p>Base Score 的各个维度，本质上是在评估：漏洞是怎么被利用的、利用难不难、需要什么权限、影响大不大。</p>
  ...
  },
{
    ...
    description: {
      "en": "A complete note on XSS, CSRF, SOP, CORS, CORS misconfigurations, and CSRF-token bypasses.",
      "zh": "保留原笔记中的 XSS、CSRF、SOP、CORS、错误配置和 Token 绕过内容，并整理为完整学习文章。"
    },
    date: "2026-05-07",
    content: {
      "en": "A complete note on XSS, CSRF, SOP, CORS, CORS misconfigurations, and CSRF-token bypasses.",
...
<p>Cross-Site Request Forgery (CSRF) and Cross-Site Scripting (XSS) 跨站点请求伪造（CSRF）和跨站点脚本（XSS）</p>
...
<p>虽然本模块所有实验环境均已禁用证书验证，但在实际应用中仍应避免使用自签名证书，因为现代浏览器可能会因 HTTPS 配置不当而拒绝通过不安全连接加载资源。有关 HTTPS 的更多详细信息，请参阅<a href="https://academy.hackthebox.com/module/details/184" target="_blank" rel="noreferrer">“HTTPS/TLS 攻击”</a>模块。</p>
...
<p>Web 服务器可以通过 CORS 在 HTTP 响应中设置以下任一 CORS 头部来配置同源策略的异常</p>
...
<p><a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Request-Method" target="_blank" rel="noreferrer">访问-控制-请求-方法</a> ：告知服务器实际请求中使用的 HTTP 方法。</p>
<p><a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Request-Headers" target="_blank" rel="noreferrer">访问-控制-请求-头部</a> ：告知服务器实际请求中使用的 HTTP 头部</p>
...
<h3>Trusted null origin, 可信的origin</h3>
...
<p>如果我们更仔细地分析 CSRF tokens，可以发现 CSRF 代币仅仅是作为 <a href="https://www.unixtimestamp.com/" target="_blank" rel="noreferrer">Unix 时间戳</a>的当前时间。这使得 CSRF 令牌具有可预测性，并允许我们创建可行的漏洞来执行 CSRF 攻击。为此，我们必须正确猜测受害者的 CSRF 令牌，即受害者在访问我们的有效载荷前最后一次访问 <code>/profile.php</code> 端点的时间。由于默认 SameSite <code>Lax</code> 策略的限制，我们无法用 JavaScript 代码动态暴力破解 CSRF 令牌，因此很难精确掌握时机。因此，我们需要在 HTML 表单中硬编码猜测的 CSRF 令牌，并更新每个猜测的值：</p>
<pre><code class="language-html">&lt;html&gt;
  &lt;body&gt;
...
xhr.send(params);</code></pre>
<h2>内容安全策略 (CSP)</h2>
<p>内容 <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP" target="_blank" rel="noreferrer">安全策略</a> (CSP) 是一种纵深防御的安全措施，它通过限制跨站脚本 (XSS) 漏洞的可利用性来降低其严重性。CSP 在<code>Content-Security-Policy</code>响应头中配置。</p>
<h3>CSP基础知识</h3>
<p>CSP 由多个指令组成。每个指令允许一个或多个值。浏览器会强制执行 CSP，并根据 CSP 阻止资源的加载或执行。本节将讨论一些示例指令。</p>
...
<p><a href="https://content-security-policy.com/#source_list" target="_blank" rel="noreferrer">有关其他 CSP 指令值，请查看此处</a>提供的列表。</p>
<hr />
<h3>安全 CSP</h3>
...
<p>我们可以使用现有的在线工具来评估云安全策略 (CSP)，例如Google 提供的<a href="https://csp-evaluator.withgoogle.com/" target="_blank" rel="noreferrer">CSP 评估</a>工具。有关如何编写安全 CSP 的更多详细信息，请参阅<a href="https://cheatsheetseries.owasp.org/cheatsheets/Content_Security_Policy_Cheat_Sheet.html" target="_blank" rel="noreferrer">OWASP CSP 速查表</a>。</p>
<h2>绕过弱CSP</h2>
<hr />
...
<p>我们有许多事件处理工具可以用于此目的。PortSwigger 的 <a href="https://portswigger.net/web-security/cross-site-scripting/cheat-sheet" target="_blank" rel="noreferrer">XSS 速查表</a>提供了很好的概述。</p>
<h3>绕过基本黑名单</h3>
...
<p>想了解更多 XSS 滤波绕过，请查看 OWASP 的 <a href="https://cheatsheetseries.owasp.org/cheatsheets/XSS_Filter_Evasion_Cheat_Sheet.html" target="_blank" rel="noreferrer">XSS 滤波规避速查表</a> 。此外，还有针对不同类型滤波器的 XSS 有效载荷集合。例如，如果无法使用括号，我们可以引用<a href="https://github.com/RenwaX23/XSS-Payloads/blob/master/Without-Parentheses.md" target="_blank" rel="noreferrer">不带括号的 XSS</a> 有效载荷集合。此外，<a href="https://html5sec.org/" target="_blank" rel="noreferrer">HTML 5 安全速查表</a>还提供了更多针对 XSS 利用的浏览器特定示例。</p>
<h2>Skills Assessment</h2>`,
      zh: String.raw`<h2>XSS 漏洞简介</h2>
<p><strong>XSS（Cross-Site Scripting）</strong> 是一种常见的 Web 安全漏洞，本质是：</p>
<blockquote>攻击者把恶意脚本（通常是 JavaScript）注入到网页中，让其他用户在浏览该页面时执行这些脚本。</blockquote>
<p>也就是说：</p>
<ul><li>攻击代码被“存”在网页或请求里</li><li>浏览器误认为是“正常内容”执行了</li><li>最终<strong>在受害者浏览器中执行攻击者的代码</strong></li></ul>
  ...
  },
{
    ...
    description: {
      "en": "A complete study note on NTLM leakage through Office, Outlook, Access, Media Player, and Publisher.",
      "zh": "保留原文案例，整理 Word、Outlook、Access、Media Player 和 Publisher 触发 NTLM 泄露的路径。"
    },
    date: "2026-05-07",
    content: {
      "en": "A complete study note on NTLM leakage through Office, Outlook, Access, Media Player, and Publisher.",
    ...
    contentHtml: {
      en: String.raw`<p>NTLM 就像过去顽固的遗物，挥之不去——这是一个有着几十年历史的身份验证协议，看似已被弃用，但仍然潜伏在每个 Windows 环境的阴影中。 尽管多年来微软一直努力用更安全的替代方案（例如 Kerberos）来取代 NTLM，但它仍然是一个关键的备用机制，微软无法完全弃用它。为什么？</p>
...
<p>**!<a href="https://www.morphisec.com/wp-content/uploads/2025/01/Picture14-ezgif.com-png-to-webp-converter.webp" target="_blank" rel="noreferrer">微软发布者警告：外部数据 NTLM 泄露</a></p>
<p><strong>为什么会发生这种情况</strong></p>
...
<p>换句话说，虽然实际获取联系人列表内容需要用户授权，但初始的文件存在性检查会触发<strong>NTLM 身份验证请求</strong>，从而导致<strong>您的 NTLM 凭据泄露</strong>。攻击者甚至不需要远程联系人列表实际存在——仅仅是尝试验证它就足以窃取您的凭据。</p>`,
      zh: String.raw`<p>NTLM 就像过去顽固的遗物，挥之不去——这是一个有着几十年历史的身份验证协议，看似已被弃用，但仍然潜伏在每个 Windows 环境的阴影中。 尽管多年来微软一直努力用更安全的替代方案（例如 Kerberos）来取代 NTLM，但它仍然是一个关键的备用机制，微软无法完全弃用它。为什么？</p>
<p>由于它深深嵌入到生态系统中，移除它可能会导致无数遗留应用程序和工作流程崩溃。攻击者正是利用这种备用依赖关系，反复使用各种技术来攻击协议固有的弱点。</p>
<h2>介绍</h2>
<p>近年来, 攻击者专注于通过NTLM泄漏漏洞来提升权限. 微软Outlook应用程序尤其成为初始访问的主要目标，因为它频繁且通常是静默的网络连接可能会触发意外的NTLM身份验证。</p>
<p>一旦攻击者获取了泄露的 NTLM 哈希值，其影响可能是毁灭性的。破解哈希值以获取明文密码并非总是必要；NTLM 哈希值本身可以直接用于<strong>哈希传递攻击</strong>。这使得攻击者无需知道用户密码即可进行身份验证，利用哈希值进行<strong>远程 PsExec、WMI 或 RDP 访问</strong>，甚至泄露之前无法获取的其他凭据。如果 NTLM 哈希值属于特权用户，攻击者可以执行<strong>DCSync 攻击</strong>、请求新的 Kerberos 票证或在域内提升权限。</p>
  ...
  },
{
    ...
    description: {
      "en": "A complete Windows privilege escalation methodology covering goals, tools, situational awareness, system, user, service, and privilege enumeration.",
      "zh": "按照原笔记结构完整整理 Windows 本地提权目标、场景、工具、态势感知、系统、用户、服务与权限枚举。"
    },
    date: "2026-05-07",
    content: {
      "en": "A complete Windows privilege escalation methodology covering goals, tools, situational awareness, system, user, service, and privilege enumeration.",
...
<p>Windows 权限提升的总体目标是将我们对特定系统的访问权限提升到组成员<code>Local Administrators</code>或<code>NT AUTHORITY\SYSTEM</code> <a href="https://docs.microsoft.com/en-us/windows/win32/services/localsystem-account" target="_blank" rel="noreferrer">本地系统</a>帐户。然而，在某些情况下，提升到系统上的其他用户就足以实现我们的目标。权限提升通常是任何攻击活动中至关重要的一步。我们需要使用获得的访问权限，或者只有在提升权限的上下文中进行会话后才能找到的某些数据（例如凭据）。在某些情况下，如果客户聘请我们进行“黄金映像”或“工作站突破”类型的评估，权限提升可能是评估的最终目标。权限提升通常对于通过网络继续实现我们的最终目标以及横向移动至关重要。</p>
<p>话虽如此，我们可能需要提升权限，原因如下：</p>
<ol><li>在测试客户端的<a href="https://www.techopedia.com/definition/29456/golden-image" target="_blank" rel="noreferrer">黄金映像</a>Windows 工作站和服务器构建是否存在缺陷时</li><li>在本地提升权限以获取对某些本地资源（例如数据库）的访问权限</li><li>在加入域的计算机上获取<a href="https://docs.microsoft.com/en-us/windows/win32/services/localsystem-account" target="_blank" rel="noreferrer">NT AUTHORITY\System</a>级别访问权限，从而进入客户端的 Active Directory 环境</li><li>获取凭证以在客户端网络内横向移动或提升权限</li></ol>
...
<p>Windows 系统存在巨大的攻击面。我们可以通过以下几种方式来提升权限：</p>
...
Password: </code></pre>
<p>模块的许多部分都需要一些工具，例如开源脚本、预编译二进制文件和漏洞利用 PoC。如果适用，这些工​​具可以在<code>C:\Tools</code>目标主机的目录中找到。尽管大多数工具都已提供，但您也可以挑战自己，尝试将文件上传到目标主机（使用<a href="https://academy.hackthebox.com/course/preview/file-transfers" target="_blank" rel="noreferrer">文件传输模块中展示的技术），甚至可以使用</a><a href="https://visualstudio.microsoft.com/downloads/" target="_blank" rel="noreferrer">Visual Studio</a>自行编译一些工具。</p>
<p><strong>Useful Tools</strong></p>
<div class="post-table-wrap"><table><thead><tr><th>Tool</th><th>Description</th></tr></thead><tbody><tr><td><a href="https://github.com/GhostPack/Seatbelt" target="_blank" rel="noreferrer">Seatbelt</a></td><td>用于执行各种本地权限提升检查的 C# 项目</td></tr><tr><td><a href="https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite/tree/master/winPEAS" target="_blank" rel="noreferrer">winPEAS</a></td><td>WinPEAS 是一个脚本，用于在 Windows 主机上搜索可能的提权路径。所有检查的说明<a href="https://book.hacktricks.wiki/en/windows-hardening/checklist-windows-privilege-escalation.html" target="_blank" rel="noreferrer">如下：</a></td></tr><tr><td><a href="https://raw.githubusercontent.com/PowerShellMafia/PowerSploit/master/Privesc/PowerUp.ps1" target="_blank" rel="noreferrer">PowerUp</a></td><td>用于查找依赖于错误配置的常见 Windows 提权向量的 PowerShell 脚本。它还可以用来利用已发现的一些问题。</td></tr><tr><td><a href="https://github.com/GhostPack/SharpUp" target="_blank" rel="noreferrer">SharpUp</a></td><td>C# 版本的 PowerUp</td></tr><tr><td><a href="https://github.com/411Hall/JAWS" target="_blank" rel="noreferrer">JAWS</a></td><td>用 PowerShell 2.0 编写的用于枚举权限提升向量的 PowerShell 脚本</td></tr><tr><td><a href="https://github.com/Arvanaghi/SessionGopher" target="_blank" rel="noreferrer">SessionGopher</a></td><td>SessionGopher 是一款 PowerShell 工具，用于查找并解密远程访问工具保存的会话信息。它可以提取 PuTTY、WinSCP、SuperPuTTY、FileZilla 和 RDP 保存的会话信息。</td></tr><tr><td><a href="https://github.com/rasta-mouse/Watson" target="_blank" rel="noreferrer">Watson</a></td><td>Watson 是一个 .NET 工具，旨在枚举缺失的 KB 并建议利用权限提升漏洞。</td></tr><tr><td><a href="https://github.com/AlessandroZ/LaZagne" target="_blank" rel="noreferrer">LaZagne</a></td><td>用于从 Web 浏览器、聊天工具、数据库、Git、电子邮件、内存转储、PHP、系统管理工具、无线网络配置、内部 Windows 密码存储机制等检索存储在本地计算机上的密码的工具</td></tr><tr><td><a href="https://github.com/bitsadmin/wesng" target="_blank" rel="noreferrer">Windows Exploit Suggester - Next Generation</a></td><td>WES-NG 是一款基于 Windows 实用程序输出的工具<code>systeminfo</code>，它提供了操作系统易受攻击的漏洞列表，以及针对这些漏洞的任何利用方式。它支持 Windows XP 到 Windows 10 之间的所有 Windows 操作系统，包括其对应的 Windows Server 版本。</td></tr><tr><td><a href="https://docs.microsoft.com/en-us/sysinternals/downloads/sysinternals-suite" target="_blank" rel="noreferrer">Sysinternals Suite</a></td><td>我们将在枚举中使用 Sysinternals 的几种工具，包括<a href="https://docs.microsoft.com/en-us/sysinternals/downloads/accesschk" target="_blank" rel="noreferrer">AccessChk</a>、<a href="https://docs.microsoft.com/en-us/sysinternals/downloads/pipelist" target="_blank" rel="noreferrer">PipeList</a>和<a href="https://docs.microsoft.com/en-us/sysinternals/downloads/psservice" target="_blank" rel="noreferrer">PsService</a></td></tr></tbody></table></div>
<h2>Getting the Lay of the Land</h2>
<h3>态势感知</h3>
<p>无论身处何种境地，无论是在日常生活中，还是在网络渗透测试等项目中，时刻把握时间和空间的定位都至关重要。</p>
<p>收集网络信息是我们枚举的关键部分。我们可能会发现主机是双宿主的，而攻陷该主机可能使我们能够横向移动到之前无法访问的网络的另一部分。双宿主意味着主机或服务器属于两个或多个不同的网络，并且在大多数情况下具有多个虚拟或物理网络接口。我们应该始终查看<a href="https://en.wikipedia.org/wiki/Routing_table" target="_blank" rel="noreferrer">路由表</a>，以查看有关本地网络及其周围网络的信息。我们还可以收集有关本地域的信息（如果主机是Active Directory环境的一部分），包括域控制器的IP地址。使用arp<a href="https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/arp" target="_blank" rel="noreferrer">命令</a>查看每个接口的ARP缓存并查看主机最近与之通信的其他主机也很重要。这可以帮助我们在获取凭据后进行横向移动。它可以很好地指示管理员正在通过RDP或WinRM从该主机连接到哪些主机。</p>
...
<p>|                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | | The highly privileged <code>NT AUTHORITY\SYSTEM</code> account, or <a href="https://docs.microsoft.com/en-us/windows/win32/services/localsystem-account" target="_blank" rel="noreferrer">LocalSystem</a> account which is a highly privileged account with more privileges than a local administrator account and is used to run most Windows services.  &lt;br&gt;高度特权的 <code>NT AUTHORITY\SYSTEM</code> 账户，或称 <a href="https://docs.microsoft.com/en-us/windows/win32/services/localsystem-account" target="_blank" rel="noreferrer">LocalSystem</a> 账户，这是一个拥有比本地管理员账户更多权限的高度特权账户，用于运行大多数 Windows 服务。 | | The built-in local <code>administrator</code> account. Some organizations disable this account, but many do not. It is not uncommon to see this account reused across multiple systems in a client environment.  &lt;br&gt;内置的本地<code>管理员</code>账户。有些组织禁用了该账户，但许多组织不会。在客户端环境中，该账户在多个系统间重复使用并不罕见。                                                                                                                                                                                                                 | | Another local account that is a member of the local <code>Administrators</code> group. Any account in this group will have the same privileges as the built-in <code>administrator</code> account.  &lt;br&gt;另一个本地账户，是本地<code>管理员</code>组的成员。该组中的任何账户都将拥有与内置<code>管理员</code>账户相同的权限。                                                                                                                                                                                                                                                  | | A standard (non-privileged) domain user who is part of the local <code>Administrators</code> group.  &lt;br&gt;一个标准（非特权）域用户，属于本地<code>管理员</code>组。                                                                                                                                                                                                                                                                                                                                                               | | A domain admin (highly privileged in the Active Directory environment) that is part of the local <code>Administrators</code> group.  &lt;br&gt;一个域管理员（在 Active Directory 环境中拥有高度权限），属于本地<code>管理员组</code> 。                                                                                                                                                                                                                                                                                                      |</p>
<p><strong>关键数据点</strong></p>
<ul><li><code>OS name</code> <code>系统名称</code> ：了解 Windows 作系统的类型（工作站或服务器）和级别（Windows 7 或 10，Server 2008、2012、2016、2019 等）可以让我们了解遗留系统中可能可用的工具类型（如 <code>PowerShell</code> 版本），或是否缺乏这些工具。这也能识别可能存在公开漏洞利用的作系统版本。</li><li><code>Version</code>: 与作系统<a href="https://en.wikipedia.org/wiki/Comparison_of_Microsoft_Windows_versions" target="_blank" rel="noreferrer">版本</a>类似，可能存在针对特定 Windows 版本漏洞的公开漏洞利用。Windows 系统漏洞可能导致系统不稳定甚至彻底崩溃。在任何生产系统上运行这些程序时都要小心，确保在运行前充分了解漏洞及其可能的后果。</li><li><code>Running Services</code> <code>运行服务</code> ：了解主机上运行的服务很重要，尤其是那些以 <code>NT AUTHORITY\SYSTEM</code> 或管理员级账户运行的服务。在特权账户中运行的服务配置错误或易受攻击，往往是权限升级的轻松优势。</li></ul>
<h4>系统信息</h4>
...
<p>熟悉标准的 Windows 进程，如<a href="https://en.wikipedia.org/wiki/Session_Manager_Subsystem" target="_blank" rel="noreferrer">会话管理器子系统（smss.exe）、</a> <a href="https://en.wikipedia.org/wiki/Client/Server_Runtime_Subsystem" target="_blank" rel="noreferrer">客户端服务器运行时子系统（csrss.exe）、</a><a href="https://en.wikipedia.org/wiki/Winlogon" target="_blank" rel="noreferrer">WinLogon（winlogon.exe）、</a> <a href="https://en.wikipedia.org/wiki/Local_Security_Authority_Subsystem_Service" target="_blank" rel="noreferrer">本地安全机构子系统服务（LSASS）</a> 和<a href="https://en.wikipedia.org/wiki/Svchost.exe" target="_blank" rel="noreferrer">服务主机（svchost.exe）</a> 等，以及与之相关的服务，是至关重要的。能够快速识别标准流程/服务将有助于加快我们的枚举速度，并使我们能够聚焦非标准流程/服务，从而可能开启权限升级的路径。在上述示例中，我们最感兴趣的是 <code>FileZilla</code> FTP 服务器的运行情况，并尝试枚举版本以查找公开漏洞或错误配置，如 FTP 匿名访问，这些可能导致敏感数据暴露甚至更多。</p>
...
<p>除了 PATH， <code>set</code>还可以提供其他有用信息，比如 HOME DRIVE。在企业中，这通常是文件共享。直接进入文件共享页面可能会显示其他可访问的目录。能够访问“IT 目录”并不罕见，该目录包含包含密码的库存表格。此外，共享用于家庭目录，用户可以登录其他电脑，拥有相同的体验/文件/桌面等（ <a href="https://docs.microsoft.com/en-us/windows-server/storage/folder-redirection/folder-redirection-rup-overview" target="_blank" rel="noreferrer">漫游配置文件</a> ）。这也可能意味着用户会携带恶意物品。如果文件被放置在 <code>USERPROFILE\AppData\Microsoft\Windows\Start Menu\Programs\Startup</code> 中，当用户登录另一台机器时，该文件将被执行。</p>
...
<p><strong>查看详细配置信息</strong> <code>systeminfo</code> 命令会显示该设备是否最近被打过修补，以及它是虚拟机。如果设备最近没有被修补，获得管理员级别访问权限可能只需运行已知的漏洞利用即可。谷歌热<a href="https://www.catalog.update.microsoft.com/Search.aspx?q=hotfix" target="_blank" rel="noreferrer">修复</a>下安装的知识库，了解盒子什么时候被修补。这些信息并不总是存在，因为热修复软件可以对非管理员隐藏。还可以查看<code>系统启动时间</code>和<code>作系统版本</code> ，以了解补丁级别。如果盒子六个月以上没有重启，很可能也没有被修补。</p>
...
<p><strong>补丁与更新</strong> 如果 <code>systeminfo</code> 不显示热修复，可以通过 <a href="https://docs.microsoft.com/en-us/windows/win32/wmisdk/wmi-start-page" target="_blank" rel="noreferrer">WMI-Command</a> 二进制和 <a href="https://docs.microsoft.com/en-us/windows/win32/cimwin32prov/win32-quickfixengineering" target="_blank" rel="noreferrer">QFE（快速修复工程）</a> 来查询补丁。</p>
...
<p>我们也可以用 <a href="https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.management/get-hotfix?view=powershell-7.1" target="_blank" rel="noreferrer">PowerShell 通过 Get-Hotfix</a> cmdlet 实现这一点。</p>
...
&lt;SNIP&gt;</code></pre>
<p>当然，我们也可以用 <a href="https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.management/get-wmiobject?view=powershell-5.1" target="_blank" rel="noreferrer">PowerShell 使用 Get-WmiObject</a> cmdlet 实现这一点。</p>
...
<p><strong>Access Tokens 访问令牌</strong> 在 Windows 中， <a href="https://docs.microsoft.com/en-us/windows/win32/secauthz/access-tokens" target="_blank" rel="noreferrer">访问令牌</a>用于描述进程或线程的安全上下文（安全属性或规则）。令牌包含用户账户身份信息以及与特定进程或线程相关的权限。当用户向系统进行身份验证时，其密码会通过安全数据库进行验证，如果正确认证，他们将获得一个访问令牌。每当用户与进程交互时，都会显示该令牌的副本以确定其权限等级。</p>
<h3>枚举网络服务</h3>
...
<p>Cobalt Strike 这样做是因为如果被执行的命令被杀毒软件标记或崩溃，它不会影响信标（执行该命令的进程）。Cobalt Strike 用户经常会更换命名的管道，伪装成另一个程序。最常见的例子之一是用 mojo 代替 msagent。我最喜欢的发现之一是找到了一个带有 Mojo 的命名管道启动器，但电脑本身并没有安装 Chrome。幸运的是，这正是公司内部的红队。当外部顾问找到红队，而内部蓝队却没有时，这说明了很多问题。</p>
<p>管道用于两个应用程序或进程之间的通信，使用共享内存。管道分为两种类型， <a href="https://docs.microsoft.com/en-us/windows/win32/ipc/named-pipes" target="_blank" rel="noreferrer">命名管道</a>和匿名管道。命名管道的一个例子是 <code>\\.\PipeName\\ExampleNamedPipeServer</code> 。Windows 系统使用客户端-服务器实现管道通信。在这种实现中，创建命名管道的进程是服务器，与命名管道通信的进程是客户端。命名管道可以通过<code>半双工</code>通信，或单向通道，客户端只能将数据写入服务器;或双<code>工</code> ，这是一种双向通信通道，允许客户端通过管道写入数据，服务器通过该管道回应数据。每一次与命名管道服务器的活跃连接都会生成一个新的命名管道。这些管道名称相同，但通过不同的数据缓冲区进行通信。</p>
...
<h2>Windows 用户权限</h2>
<p>Windows 中的<a href="https://docs.microsoft.com/en-us/windows/win32/secauthz/privileges" target="_blank" rel="noreferrer">权限</a>是指账户可以被授予在本地系统上执行各种作的权利，比如管理服务、加载驱动程序、关闭系统、调试应用程序等。权限不同于访问权，后者是系统用来授予或拒绝访问可保护对象的。用户和组权限存储在数据库中，用户登录系统时通过访问令牌授予。账户可以对特定计算机拥有本地权限，如果账户属于 Active Directory 域，则在不同系统上拥有不同的权限。每当用户尝试执行特权作时，系统会检查该用户的访问令牌，以确认账户是否具备所需权限，如果有，会检查这些令牌是否已被启用。大多数权限默认是被禁用的。有些可以通过打开管理 cmd.exe 或 PowerShell 控制台来启用，而另一些则可以通过手动启用。</p>
<p>评估的目标通常是获得对一个或多个系统的管理访问权限。假设我们可以以特定权限用户身份登录系统。在这种情况下，我们或许可以利用内置功能直接提升权限，或者利用目标账户分配的权限来进一步提升访问权限，以实现最终目标。</p>
<p><strong>Windows授权流程</strong> 安全主体是指任何可以被 Windows 作系统认证的对象，包括用户账户和计算机账户、在安全上下文中运行的进程，或其他用户/计算机账户，或这些账户所属的安全组。安全主体是控制 Windows 主机资源访问的主要方式。每个安全主体都由唯一的<a href="https://docs.microsoft.com/en-us/troubleshoot/windows-server/identity/security-identifiers-in-windows" target="_blank" rel="noreferrer">安全标识符（SID）</a> 标识。当创建安全主体时，会被分配一个 SID，该 SID 在其生命周期内一直分配给该主体。</p>
<p>下图从高层次讲解了 Windows 授权和访问控制流程，例如，当用户尝试访问文件共享中的可安全对象（如文件夹）时，该过程就开始了。在此过程中，用户的访问令牌（包括其用户 SID、其所属组的 SID、权限列表及其他访问信息）会与对象<a href="https://docs.microsoft.com/en-us/windows/win32/secauthz/security-descriptors" target="_blank" rel="noreferrer">安全描述符</a>内的<a href="https://docs.microsoft.com/en-us/windows/win32/secauthz/access-control-entries" target="_blank" rel="noreferrer">访问控制条目（ACEs）进行比较（ACEs，ACEs</a> 包含可保护对象的安全信息，如授予用户或组的访问权，下文将讨论）。一旦比较完成，就会决定是否授予访问。每当用户尝试访问 Windows 主机上的资源时，整个过程几乎瞬间发生。作为我们列举和特权提升活动的一部分，我们试图利用和滥用访问权，利用或介入该授权流程，以进一步推进目标的访问。</p>
...
<div class="post-table-wrap"><table><thead><tr><th><strong>Group  集团</strong></th><th><strong>Description  描述</strong></th></tr></thead><tbody><tr><td>Default Administrators  默认管理员</td><td>Domain Admins and Enterprise Admins are "super" groups.  &lt;br&gt;域管理员和企业管理员是“超级”组。</td></tr><tr><td>Server Operators  服务器运营者</td><td>Members can modify services, access SMB shares, and backup files.  &lt;br&gt;成员可以修改服务、访问 SMB 共享和备份文件。</td></tr><tr><td>Backup Operators  备用</td><td>Members are allowed to log onto DCs locally and should be considered Domain Admins. They can make shadow copies of the SAM/NTDS database, read the registry remotely, and access the file system on the DC via SMB. This group is sometimes added to the local Backup Operators group on non-DCs.  &lt;br&gt;成员可以本地登录 DC，应被视为域管理员。他们可以复制 SAM/NTDS 数据库的影子副本，远程读取注册表，并通过 SMB 访问 DC 上的文件系统。该组有时会加入非 DC 的本地备份组。</td></tr><tr><td>Print Operators  打印</td><td>Members can log on to DCs locally and "trick" Windows into loading a malicious driver.  &lt;br&gt;成员可以登录本地的 DC，并“欺骗”Windows 加载恶意驱动程序。</td></tr><tr><td>Hyper-V Administrators  Hyper-V 管理员</td><td>If there are virtual DCs, any virtualization admins, such as members of Hyper-V Administrators, should be considered Domain Admins.  &lt;br&gt;如果存在虚拟数据中心，任何虚拟化管理员，如 Hyper-V 管理员成员，都应被视为域管理员。</td></tr><tr><td>Account Operators  账户运营方</td><td>Members can modify non-protected accounts and groups in the domain.  &lt;br&gt;成员可以修改域内非受保护的账户和组。</td></tr><tr><td>Remote Desktop Users  远程桌面用户</td><td>Members are not given any useful permissions by default but are often granted additional rights such as <code>Allow Login Through Remote Desktop Services</code> and can move laterally using the RDP protocol.  &lt;br&gt;成员默认没有任何有用的权限，但通常会被赋予额外权限 <code>Allow Login Through Remote Desktop Services</code> ，比如，并且可以通过 RDP 协议进行横向移动。</td></tr><tr><td>Remote Management Users  远程管理用户</td><td>Members can log on to DCs with PSRemoting (This group is sometimes added to the local remote management group on non-DCs).  &lt;br&gt;成员可以通过 PSRemoting 登录 DC（该组有时会加入非 DC 的本地远程管理组）。</td></tr><tr><td>Group Policy Creator Owners  &lt;br&gt;组策略创建者所有者</td><td>Members can create new GPOs but would need to be delegated additional permissions to link GPOs to a container such as a domain or OU.  &lt;br&gt;成员可以创建新的 GPO，但需要被授权额外权限才能将 GPO 关联到容器，如域或 OU。</td></tr><tr><td>Schema Admins  模式管理员</td><td>Members can modify the Active Directory schema structure and backdoor any to-be-created Group/GPO by adding a compromised account to the default object ACL.  &lt;br&gt;成员可以通过在默认对象 ACL 中添加被攻破的账户来修改 Active Directory 的模式结构，并对任何待创建的 Group/GPO 进行背门。</td></tr><tr><td>DNS Admins  DNS 管理员</td><td>Members can load a DLL on a DC, but do not have the necessary permissions to restart the DNS server. They can load a malicious DLL and wait for a reboot as a persistence mechanism. Loading a DLL will often result in the service crashing. A more reliable way to exploit this group is to <a href="https://web.archive.org/web/20231115070425/https://cube0x0.github.io/Pocing-Beyond-DA/" target="_blank" rel="noreferrer">create a WPAD record</a>.  &lt;br&gt;成员可以在 DC 上加载 DLL，但没有重启 DNS 服务器的必要权限。他们可以加载恶意 DLL，等待重启作为持久化机制。加载 DLL 通常会导致服务崩溃。利用该群体的更可靠方法是<a href="https://web.archive.org/web/20231115070425/https://cube0x0.github.io/Pocing-Beyond-DA/" target="_blank" rel="noreferrer">创建一个 WPAD 记录</a> 。</td></tr></tbody></table></div>
<p><strong>用户权利转让</strong> 根据组成员身份以及通过域和本地组策略分配的权限等其他因素，用户可能会为其账户分配各种权限。这篇关于<a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/user-rights-assignment" target="_blank" rel="noreferrer">用户权利转让</a>的 Microsoft 文章详细说明了 Windows 中可设置的每种用户权限以及适用于每个权限的安全考虑。以下是一些关键的用户权限分配，这些设置是应用于本地主机的。这些权限允许用户在系统上执行任务，如本地或远程登录、从网络访问主机、关闭服务器等。</p>
<div class="post-table-wrap"><table><thead><tr><th>Setting <a href="https://docs.microsoft.com/en-us/windows/win32/secauthz/privilege-constants" target="_blank" rel="noreferrer">Constant</a>  &lt;br&gt;设定<a href="https://docs.microsoft.com/en-us/windows/win32/secauthz/privilege-constants" target="_blank" rel="noreferrer">常数</a></th><th>Setting Name  设定名称</th><th>Standard Assignment  标准作业</th><th>Description  描述</th></tr></thead><tbody><tr><td>SeNetworkLogonRight</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/access-this-computer-from-the-network" target="_blank" rel="noreferrer">Access this computer from the network  &lt;br&gt;从网络访问这台电脑</a></td><td>Administrators, Authenticated Users  &lt;br&gt;管理员，认证用户</td><td>Determines which users can connect to the device from the network. This is required by network protocols such as SMB, NetBIOS, CIFS, and COM+.  &lt;br&gt;决定哪些用户可以从网络连接到该设备。这被 SMB、NetBIOS、CIFS 和 COM+等网络协议要求。</td></tr><tr><td>SeRemoteInteractiveLogonRight</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/allow-log-on-through-remote-desktop-services" target="_blank" rel="noreferrer">Allow log on through Remote Desktop Services  &lt;br&gt;允许通过远程桌面服务登录</a></td><td>Administrators, Remote Desktop Users  &lt;br&gt;管理员，远程桌面用户</td><td>This policy setting determines which users or groups can access the login screen of a remote device through a Remote Desktop Services connection. A user can establish a Remote Desktop Services connection to a particular server but not be able to log on to the console of that same server.  &lt;br&gt;该策略设置决定哪些用户或组可以通过远程桌面服务连接访问远程设备的登录界面。用户可以建立与特定服务器的远程桌面服务连接，但无法登录该服务器的控制台。</td></tr><tr><td>SeBackupPrivilege  SeeBackupPrivilege</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/back-up-files-and-directories" target="_blank" rel="noreferrer">Back up files and directories  &lt;br&gt;备份文件和目录</a></td><td>Administrators  管理人员</td><td>This user right determines which users can bypass file and directory, registry, and other persistent object permissions for the purposes of backing up the system.  &lt;br&gt;该用户权限决定哪些用户可以绕过文件和目录、注册表及其他持久对象权限以备份系统。</td></tr><tr><td>SeSecurityPrivilege</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/manage-auditing-and-security-log" target="_blank" rel="noreferrer">Manage auditing and security log  &lt;br&gt;管理审计和安全日志</a></td><td>Administrators  管理人员</td><td>This policy setting determines which users can specify object access audit options for individual resources such as files, Active Directory objects, and registry keys. These objects specify their system access control lists (SACL). A user assigned this user right can also view and clear the Security log in Event Viewer.  &lt;br&gt;该策略设置决定哪些用户可以为单个资源（如文件、Active Directory 对象和注册表键）指定对象访问审计选项。这些对象指定其系统访问控制列表（SACL）。被赋予该用户权限的用户也可以在事件查看器中查看并清除安全日志。</td></tr><tr><td>SeTakeOwnershipPrivilege  &lt;br&gt;SeTakeOwnership 特权</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/take-ownership-of-files-or-other-objects" target="_blank" rel="noreferrer">Take ownership of files or other objects  &lt;br&gt;拥有文件或其他对象</a></td><td>Administrators  管理人员</td><td>This policy setting determines which users can take ownership of any securable object in the device, including Active Directory objects, NTFS files and folders, printers, registry keys, services, processes, and threads.  &lt;br&gt;该策略设置决定哪些用户可以拥有设备中任何可保护的对象，包括 Active Directory 对象、NTFS 文件和文件夹、打印机、注册表键、服务、进程和线程。</td></tr><tr><td>SeDebugPrivilege</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/debug-programs" target="_blank" rel="noreferrer">Debug programs  调试程序</a></td><td>Administrators  管理人员</td><td>This policy setting determines which users can attach to or open any process, even a process they do not own. Developers who are debugging their applications do not need this user right. Developers who are debugging new system components need this user right. This user right provides access to sensitive and critical operating system components.  &lt;br&gt;该策略设置决定哪些用户可以连接或打开任何进程，即使是他们不拥有的进程。调试应用程序的开发者不需要这个用户权限。调试新系统组件的开发者需要这个用户权限。该用户权利允许访问敏感且关键的作系统组件。</td></tr><tr><td>SeImpersonatePrivilege  冒充特权</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/impersonate-a-client-after-authentication" target="_blank" rel="noreferrer">Impersonate a client after authentication  &lt;br&gt;认证后冒充客户端</a></td><td>Administrators, Local Service, Network Service, Service  &lt;br&gt;管理员，本地服务，网络服务，服务</td><td>This policy setting determines which programs are allowed to impersonate a user or another specified account and act on behalf of the user.  &lt;br&gt;该策略设置决定哪些程序可以冒充用户或其他指定账户并代表用户行动。</td></tr><tr><td>SeLoadDriverPrivilege</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/load-and-unload-device-drivers" target="_blank" rel="noreferrer">Load and unload device drivers  &lt;br&gt;加载和卸载设备驱动程序</a></td><td>Administrators  管理人员</td><td>This policy setting determines which users can dynamically load and unload device drivers. This user right is not required if a signed driver for the new hardware already exists in the driver.cab file on the device. Device drivers run as highly privileged code.  &lt;br&gt;该策略设置决定哪些用户可以动态加载和卸载设备驱动程序。如果设备上的 driver.cab 文件中已有新硬件的签名驱动，则无需使用此用户权利。设备驱动程序作为高权限代码运行。</td></tr><tr><td>SeRestorePrivilege  SeeRestorePrivilege</td><td><a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/restore-files-and-directories" target="_blank" rel="noreferrer">Restore files and directories  &lt;br&gt;还原文件和目录</a></td><td>Administrators  管理人员</td><td>This security setting determines which users can bypass file, directory, registry, and other persistent object permissions when they restore backed up files and directories. It determines which users can set valid security principals as the owner of an object.  &lt;br&gt;该安全设置决定哪些用户在恢复备份文件和目录时可以绕过文件、目录、注册表及其他持久对象权限。它决定哪些用户可以作为对象的所有者设置有效的安全主体。</td></tr><tr><td>SeTcbPrivilege  SeTcb 特权</td><td><a href="https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-10/security/threat-protection/security-policy-settings/act-as-part-of-the-operating-system" target="_blank" rel="noreferrer">Act as part of the operating system  &lt;br&gt;作为作系统的一部分</a></td><td>Administrators, Local Service, Network Service, Service  &lt;br&gt;管理员，本地服务，网络服务，服务</td><td>This security setting determines whether a process can assume the identity of any user and, through this, obtain access to resources that the targeted user is permitted to access (impersonation). This may be assigned to antivirus or backup tools that need the ability to access all system files for scans or backups. This privilege should be reserved for service accounts requiring this access for legitimate activities.  &lt;br&gt;该安全设置决定进程是否可以冒充任何用户身份，并通过此获取目标用户被允许访问的资源（冒充）。这可能被分配给杀毒或备份工具，需要访问所有系统文件进行扫描或备份。该权限应保留给需要合法访问的服务账户。</td></tr></tbody></table></div>
<p>输入命令 <code>whoami /priv</code> 会显示分配给你当前用户的所有用户权限列表。有些权限仅对管理员用户开放，且只能在运行提升级的命令或 PowerShell 会话时列出或利用。这些提升权限和<a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/user-account-control/how-user-account-control-works" target="_blank" rel="noreferrer">用户账户控制（UAC）</a> 的概念是 Windows Vista 引入的安全功能，默认限制应用程序在非必要时无法完全权限运行。如果我们比较管理员在非提升控制台和升级控制台上可享有的权利，会发现它们差别很大。</p>
...
<p>当我们的账户在 <code>“禁用</code> ”状态下显示有权限时，说明我们的账户拥有该特权。不过，在启用之前，它不能用于访问令牌来执行相关作。Windows 没有内置命令或 PowerShell cmdlet 来启用权限，所以我们需要一些脚本来帮忙。在本模块中，我们将看到滥用各种特权的方法，以及在当前流程中实现特定特权的各种方式。一个例子是这个 PowerShell <a href="https://www.powershellgallery.com/packages/PoshPrivilege/0.3.0.0/Content/Scripts%5CEnable-Privilege.ps1" target="_blank" rel="noreferrer">脚本</a> ，可以用来启用某些权限， <a href="https://www.leeholmes.com/adjusting-token-privileges-in-powershell/" target="_blank" rel="noreferrer">或者这个脚本</a>可以用来调整令牌权限。</p>
<p>相比之下，普通用户权限很少</p>
...
<p>我们通常会在通过在服务账户上下文中运行的应用程序获得远程代码执行后获得此权限（例如，将网页壳上传到 ASP.NET 的网页应用，通过 Jenkins 安装实现远程代码执行，或通过 MSSQL 查询执行命令）。每当我们通过这种方式获得访问权限时，应立即检查是否有权限，因为它的存在通常为获得更高权限提供了快速且便捷的途径。本文<a href="https://github.com/hatRiot/token-priv/blob/master/abusing_token_eop_1.0.txt" target="_blank" rel="noreferrer">值得一</a>读，以了解更多关于代币冒充攻击的细节。</p>
<p><strong>Selmpersonate Example - JuicyPotato</strong> 我们以下面的例子为例，我们通过一个特权 SQL 用户在 SQL 服务器上站稳脚跟。客户端连接到 IIS 和 SQL Server 时可以配置为使用 Windows 认证。服务器随后可能需要访问其他资源，如文件共享，作为连接客户端。这可以通过冒充客户端连接所建立的上下文用户来实现。为此，服务账户将在认证权限后获得“ <a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/impersonate-a-client-after-authentication" target="_blank" rel="noreferrer">冒充客户端</a> ”的授权。</p>
...
<p><strong>作为SYSTEM的远程代码执行</strong> 我们也可以利用 <code>SeDebugPrivilege</code> 来实现 <a href="https://decoder.cloud/2018/02/02/getting-system/" target="_blank" rel="noreferrer">RCE</a>。利用这种技术，我们可以通过启动<a href="https://docs.microsoft.com/en-us/windows/win32/procthread/child-processes" target="_blank" rel="noreferrer">子进程</a> ，利用通过 <code>SeDebugPrivilege</code> 赋予账户的提升权限，改变正常系统行为，继承<a href="https://docs.microsoft.com/en-us/windows/win32/procthread/processes-and-threads" target="_blank" rel="noreferrer">父进程</a>的令牌并冒充它，从而将权限提升到 SYSTEM。如果我们以 SYSTEM 形式运行的父进程（指定目标进程或运行程序的进程 ID（或 PID），那么我们可以快速提升权限。让我们看看实际作。</p>
<p>首先，将这个 <a href="https://raw.githubusercontent.com/decoder-it/psgetsystem/master/psgetsys.ps1" target="_blank" rel="noreferrer">PoC 脚本</a>传输到目标系统。接下来我们只需加载脚本，并用以下语法 <code>[MyProcess]::CreateProcessFromParent(&lt;system_pid&gt;,&lt;command_to_execute&gt;,"")</code> 运行。注意，我们必须在末尾添加第三个空参数 <code>""</code>，这样 PoC 才能正常工作。</p>
...
<p>还有类似<a href="https://github.com/daem0nc0re/PrivFu/tree/main/PrivilegedOperations/SeDebugPrivilegePoC" target="_blank" rel="noreferrer">这样的</a>工具，可以在我们有 <code>SeDebugPrivilege</code> 时弹出 SYSTEM shell。通常我们无法通过 RDP 访问主机，因此必须修改 PoC，要么将反向 shell 返回攻击主机，作为 SYSTEM，要么通过其他命令，比如添加管理员用户。试着玩玩这些 PoC，看看还有什么其他方式可以实现 SYSTEM 访问，尤其是当你没有完全交互式的会话时，比如实现命令注入，或者作为 <code>SeDebugPrivilege</code> 的用户拥有网页壳或反向 shell 连接。请记住这些例子，以防你遇到倾销 LSASS 无法获得有用凭证的情况（虽然我们可以通过机器的 NTLM 哈希获得 SYSTEM 访问权限，但这超出本模块范围），并且用 shell 或 RCE 作为 SYSTEM 会很有帮助。</p>
...
SeChangeNotifyPrivilege       Bypass traverse checking                                Enabled
SeIncreaseWorkingSetPrivilege Increase a process working set                          Disabled</code></pre>
<p>从输出中注意，该权限未被启用。我们可以<a href="https://raw.githubusercontent.com/fashionproof/EnableAllTokenPrivs/master/EnableAllTokenPrivs.ps1" target="_blank" rel="noreferrer">使用这个脚本</a>来启用它，这个脚本在<a href="https://www.leeholmes.com/blog/2010/09/24/adjusting-token-privileges-in-powershell/" target="_blank" rel="noreferrer">这</a>篇博客文章中有详细介绍，也可以用<a href="https://medium.com/@markmotig/enable-all-token-privileges-a7d21b1a4a77" target="_blank" rel="noreferrer">这个</a>脚本，它基于最初的概念展开。</p>
...
<p>如 <code>Windows 权限概览</code>部分所述，Windows 服务器，尤其是域控制器，内置了多种组，这些组要么随作系统自带，要么在系统安装 Active Directory 域服务角色以将服务器升级为域控制器时添加。许多这些组织会赋予成员特殊权限，有些甚至可以用于提升服务器或域控制器的权限。 <a href="https://ss64.com/nt/syntax-security_groups.html" target="_blank" rel="noreferrer">这里</a>列出了所有内置的 Windows 组，并附有每个组的详细描述。本<a href="https://docs.microsoft.com/en-us/windows-server/identity/ad-ds/plan/security-best-practices/appendix-b--privileged-accounts-and-groups-in-active-directory" target="_blank" rel="noreferrer">页面</a>详细列出了 Active Directory 中特权账户和组的列表。无论我们是否访问了其中一个或多个成员账户，或在评估过程中发现自己在其中一个或多个群体中存在过多/不必要的成员身份，都必须理解这些群体成员身份的影响。在我们的目的上，我们将重点介绍以下内置组。这些组从 Server 2008 R2 一直存在至今，除了 Hyper-V 管理员（由 Server 2012 引入）。</p>
...
<p>|<a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#bkmk-backupoperators" target="_blank" rel="noreferrer">Backup Operators  备用</a>|<a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#bkmk-eventlogreaders" target="_blank" rel="noreferrer">Event Log Readers  事件日志阅读器</a>|<a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#bkmk-dnsadmins" target="_blank" rel="noreferrer">DnsAdmins  DNS 管理员</a>| |<a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#bkmk-hypervadministrators" target="_blank" rel="noreferrer">Hyper-V Administrators  Hyper-V 管理员</a>|<a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#bkmk-printoperators" target="_blank" rel="noreferrer">Print Operators  打印</a>|<a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#bkmk-serveroperators" target="_blank" rel="noreferrer">Server Operators  服务器运营者</a>|</p>
...
<p>假设启用了对进程创建事件及相应命令行<a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/auditing/audit-process-creation" target="_blank" rel="noreferrer">值的审计</a> 。此时，该信息会作为事件 ID 4688 保存到 Windows 安全事件日志<a href="https://docs.microsoft.com/en-us/windows/security/threat-protection/auditing/event-4688" target="_blank" rel="noreferrer">中：新进程已创建</a> 。组织可以支持进程命令行的日志记录，帮助防御者监控和识别可能的恶意行为，识别系统中不应存在的二进制文件。这些数据可以被传输到 SIEM 工具，或导入搜索工具（如 ElasticSearch），以便防御者了解网络系统中运行的二进制文件。这些工具随后会标记任何潜在的恶意活动，比如市场营销主管工作站运行的 <code>whoami</code>、<code>netstat</code> 和<code>任务列表</code>命令。</p>
...
<p>管理员或<a href="https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dn579255\(v=ws.11\" target="_blank" rel="noreferrer">事件日志阅读</a>?redirectedfrom=MSDN#event-log-readers)组成员有权访问此日志。系统管理员可能会想将高级用户或开发者加入该组，以执行某些任务而无需授予管理员权限。</p>
...
logger
The command completed successfully.</code></pre>
<p>Microsoft 发布了涵盖所有内置 Windows 命令的参考<a href="https://download.microsoft.com/download/5/8/9/58911986-D4AD-4695-BF63-F734CD4DF8F2/ws-commands.pdf" target="_blank" rel="noreferrer">指南</a> ，包括语法、参数和示例。许多 Windows 命令支持以密码作为参数传递，如果启用了对进程命令行的审计，这些敏感信息将被捕获。</p>
...
<p>该 cmdlet 也可以作为另一个用户使用 <code>-Credential</code> 参数运行。 其他日志包括 <a href="https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_logging_windows?view=powershell-7.1" target="_blank" rel="noreferrer">PowerShell 作</a>日志，如果启用脚本块或模块日志，可能还包含敏感信息或凭证。该日志对无权限用户开放。</p>
<h3>DnsAdmins</h3>
<p><a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#dnsadmins" target="_blank" rel="noreferrer">DnsAdmins</a> 组的成员可以访问网络上的 DNS 信息。Windows DNS 服务支持自定义插件，并能调用插件中的函数来解决不在任何本地托管 DNS 区域范围内的名称查询。DNS 服务以 <code>NT AUTHORITY\SYSTEM</code> 形式运行，因此该组成员身份可能被用来提升域控制器的权限，或在有独立服务器作为该域的 DNS 服务器时升级。可以使用内置的 <a href="https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/dnscmd" target="_blank" rel="noreferrer">dnscmd</a> 工具来指定插件 DLL 的路径。正如这篇优秀<a href="https://adsecurity.org/?p=4064" target="_blank" rel="noreferrer">文章</a>中详细说明的，当域名控制器上运行 DNS 时（非常常见）可以实施以下攻击：</p>
<p>DNS 管理通过 RPC 进行</p>
...
<p>We open a listener with msfconsole using this: 我们用 msfconsole 打开一个监听器，使用这个方法：</p>
...
<p>We then get a reverse shell simultaniously: 然后我们得到一个反向壳同时：</p>
<p>Press enter or click to view image in full size</p>
...
<p><a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#print-operators" target="_blank" rel="noreferrer">打印操作员</a>组是另一个权限极高的组，它赋予其成员<code>SeLoadDriverPrivilege</code>管理、创建、共享和删除连接到域控制器的打印机的权限，以及本地登录域控制器并将其关闭的权限。如果我们发出命令<code>whoami /priv</code>，并且在非提升权限的上下文中看不到该组<code>SeLoadDriverPrivilege</code>，则需要绕过用户帐户控制 (UAC)。</p>
<ol><li>确认权限</li></ol>
<pre><code class="language-cmd-session">C:\htb&gt; whoami /priv
...
<p>UACMe<a href="https://github.com/hfiref0x/UACME" target="_blank" rel="noreferrer">代码</a>库提供了一个全面的 UAC 绕过方法列表，可从命令行使用。或者，我们也可以从图形用户界面 (GUI) 打开一个管理员命令 shell，并输入属于“打印操作员”组的帐户凭据。如果我们再次检查权限，<code>SeLoadDriverPrivilege</code>会发现该权限可见但已被禁用。</p>
<ol><li>再次确认权限</li></ol>
<pre><code class="language-cmd-session">C:\htb&gt; whoami /priv
...
SeChangeNotifyPrivilege       Bypass traverse checking             Enabled
SeIncreaseWorkingSetPrivilege Increase a process working set       Disabled</code></pre>
<p>众所周知，该驱动程序<code>Capcom.sys</code>包含允许任何用户以 SYSTEM 权限执行 shellcode 的功能。我们可以利用这些权限加载此易受攻击的驱动程序并提升权限。我们可以使用<a href="https://raw.githubusercontent.com/3gstudent/Homework-of-C-Language/master/EnableSeLoadDriverPrivilege.cpp" target="_blank" rel="noreferrer">此</a>工具加载驱动程序。该 PoC 不仅能够启用权限，还能为我们加载驱动程序。</p>
...
<p><strong>添加驱动程序引用</strong> <code>Capcom.sys</code>接下来，从<a href="https://github.com/FuzzySecurity/Capcom-Rootkit/blob/master/Driver/Capcom.sys" target="_blank" rel="noreferrer">这里</a>下载驱动程序，并将其保存到指定位置<code>C:\temp</code>。执行以下命令，在 HKEY_CURRENT_USER 树下添加对该驱动程序的引用。</p>
...
The operation completed successfully.</code></pre>
<p><code>\??\</code>用于引用恶意驱动程序映像路径的特殊语法是一种<a href="https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-even/c1550f98-a1ce-426a-9991-7509e7c3787c" target="_blank" rel="noreferrer">NT 对象路径</a>。Win32 API 将解析并解析此路径，以便正确定位并加载我们的恶意驱动程序。</p>
...
<p><strong>使用ExploitCapcom工具提升权限</strong> 要利用 Capcom.sys，我们可以先用 Visual Studio 编译，然后使用<a href="https://github.com/tandasat/ExploitCapcom" target="_blank" rel="noreferrer">ExploitCapcom工具。</a></p>
...
<p><strong>自动化步骤</strong> 使用EopLoadDriver实现自动化 <a href="https://github.com/TarlogicSecurity/EoPLoadDriver/" target="_blank" rel="noreferrer">我们可以使用EoPLoadDriver</a>之类的工具来自动执行启用权限、创建注册表项以及加载驱动程序的过程<code>NTLoadDriver</code>。为此，我们可以运行以下命令：</p>
...
<p><a href="https://docs.microsoft.com/en-us/windows/security/identity-protection/access-control/active-directory-security-groups#bkmk-serveroperators" target="_blank" rel="noreferrer">服务器运营者</a>组允许成员无需分配域管理员权限即可管理 Windows 服务器。这是一个非常高权限的组，可以本地登录服务器，包括域控制器。 加入该组可获得强大的 <code>SeBackupPrivilege</code> 和 <code>SeRestorePrivilege</code> 权限，并能控制本地服务。</p>
...
<p>- 在程序需要 <strong>管理员权限</strong> 时弹出确认提示 - 防止系统被 <strong>未经授权的程序修改</strong> 关键特点： - 默认情况下，程序都以 <strong>普通用户权限（Standard User）</strong> 运行 - 只有管理员 <strong>明确允许</strong> 时才会提升为管理员权限 - 主要目的是 <strong>减少误操作和恶意软件影响</strong> 但需要注意：</p>
<ol><li><strong>UAC（用户账户控制）</strong>是 Windows 的一项安全机制，用来：</li></ol>
<blockquote>UAC 不是严格的安全边界（not a security boundary），只是一个防护层。</blockquote>
<blockquote>攻击者如果已经在系统里，仍然可能通过 <strong>UAC Bypass</strong> 提权。</blockquote>
<h3>Weak Permissions</h3>
<p>Windows 系统的权限设置复杂且具有挑战性。一个地方的轻微修改可能会在其他地方出现缺陷。作为渗透测试人员，我们需要了解 Windows 中的权限工作原理，以及错误配置如何被利用来提升权限。本节讨论的权限相关缺陷在大型厂商发布的软件应用中相对较少见（但偶尔会出现），而在较小厂商的第三方软件、开源软件和定制应用中则很常见。服务通常以系统权限安装，因此利用服务权限相关的缺陷往往能实现对目标系统的完全控制。无论环境如何，我们都应始终检查权限薄弱，并且既能借助工具，也能手动作，以防工具不方便使用。</p>
...
<p>****</p>
<p>1. Permissive File System ACLs（可写服务文件）</p>
...
<p>这 3 个示例分别是：</p>
<p>- 低权限用户读取shadow副本里的 <code>SAM / SYSTEM / SECURITY</code> - 离线提取本地账户哈希 - 再利用哈希拿高权限</p>
<p>- 利用打印后台处理服务漏洞 - 直接添加一个本地管理员用户，或者执行恶意 DLL - 再切到高权限上下文</p>
<p>- 利用任意文件移动漏洞，把你可控的恶意程序放进 SYSTEM 服务路径 - 启动服务拿 SYSTEM shell</p>
<ol><li><strong>HiveNightmare / SeriousSam</strong></li><li><strong>PrintNightmare</strong></li><li><strong>CVE-2020-0668 + Mozilla Maintenance Service</strong></li></ol>
...
<p>Unattended installation files 可能定义了自动登录设置或安装过程中需要创建的额外账户。<code>unattend.xml</code> 中的密码以明文或 base64 编码存储。</p>
...
<p>正如 Microsoft 发布的（handy）Windows Commands PDF 中所<a href="https://download.microsoft.com/download/5/8/9/58911986-D4AD-4695-BF63-F734CD4DF8F2/ws-commands.pdf" target="_blank" rel="noreferrer">见，有</a>许多命令可以在命令行传递凭证。在下面的示例中可以看到，用户指定本地管理凭证用 <a href="https://ss64.com/nt/wevtutil.html" target="_blank" rel="noreferrer">wevutil</a> 查询应用事件日志。</p>
...
<p>PowerShell 凭据常被用于脚本编写和自动化任务，方便地存储加密凭据。凭据通过 <a href="https://en.wikipedia.org/wiki/Data_Protection_API" target="_blank" rel="noreferrer">DPAPI</a> 保护，通常意味着只有同一用户在创建它们的同一计算机上才能解密。</p>
...
<p>我们可以使用以下命令手动搜索文件系统或共享驱动器中的内容。 Search File Contents for String - Example 1</p>
...
<p>我们可以把三个 <code>plum.sqlite*</code> 文件复制到系统，用 DB <a href="https://sqlitebrowser.org/dl/" target="_blank" rel="noreferrer">浏览器等 SQLite</a> 工具打开，通过查询<code>select Text from Note;</code> 查看 Note 表中的 <code>Text</code> 列。</p>
<img src="assets/posts/windows-privilege-escalation/Pasted image 20260322052414.png" alt="Pasted image 20260322052414" />
<p><strong>使用 PowerShell 查看便签数据</strong> 这也可以用 PowerShell 的 <a href="https://github.com/RamblingCookieMonster/PSSQLite" target="_blank" rel="noreferrer">PSSQLite 模块</a>完成。首先，导入模块，指向一个数据源（这里指 StickNotes 应用使用的 SQLite 数据库文件），最后查询 <code>Note</code> 表，寻找任何有趣的数据。这也可以在我们的攻击机器上下载 <code>.sqlite</code> 文件后完成，或者远程通过 WinRM 完成。</p>
...
<p>离线破解哈希 然后我们可以将哈希输入 Hashcat，KeePass 的<a href="https://hashcat.net/wiki/doku.php?id=example_hashes" target="_blank" rel="noreferrer">哈希模式</a>为 13400。如果成功，我们可能获得大量凭证，用于访问其他应用/系统，甚至网络设备、服务器、数据库等，前提是我们能访问 IT 人员使用的密码数据库。</p>
...
<p>Autologon 账户的典型配置涉及手动设置以下注册表密钥：</p>
...
<p>如果现有快捷键文件不可用，还有其他方法可以考虑。一种选择是通过 SMB 服务器传输已有的快捷方式文件。或者，我们可以按照 <code>Generating a Malicious .lnk File</code> 标签页下“ <a href="https://academy.hackthebox.com/module/67/section/630" target="_blank" rel="noreferrer">与用户互动”部分</a>提到的，使用 PowerShell 创建一个新的快捷方式文件。这些方法在使用快捷键文件时实现目标提供了灵活性。</p>
...
<p>另外，假设我们的客户将我们置于环境中的攻击机器上。在这种情况下，值得先运行 <code>tcpdump</code> 或 <code>Wireshark</code> 一段时间，看看有哪些类型的流量通过线路传输，以及是否能发现什么有趣的情况。工具网络<a href="https://github.com/DanMcInerney/net-creds" target="_blank" rel="noreferrer">信用</a>记录可以从我们的攻击设备运行，从实时界面或 pcap 文件中检测密码和哈希值。值得在评估时让该工具在后台运行，或者用 pcap 测试，看看能否提取对权限升级或横向转移有用的凭证。</p>
